...
async function handleCopyCommand(options: any[], body: any) {
  ...
  return immediateResponse;
    ...
    const embed = {
      ...
      color: 0x00ff00,
      fields: [
        { name: "🎰 Games Added", value: `${savedGames.length}`, inline: true },
        { name: "🔗 Source", value: url, inline: false }
      ],
      footer: {
        text: "Games saved to database and will deploy automatically!"
      }
    };
    ...
    if (savedGames.length > 0) {
      embed.fields.push({
        name: "🎮 New Games",
        value: savedGames.map(g => `• ${g.name} (${g.provider})`).slice(0, 5).join('\n') + 
               (savedGames.length > 5 ? `\n... and ${savedGames.length - 5} more` : ''),
        inline: false
      });
    }
    ...
    if (error.message === 'Timeout') {
      // Send deferred response for long operations
      processScraping(url).catch(console.error);
      
      return NextResponse.json({
        type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
        data: {
          content: "⏳ **Scraping started!**\n\nThis is taking longer than expected. The scraping will continue in the background and games will be added to your database automatically.\n\n🔍 Scraping: " + url,
        }
      });
    }
